type nonrec t('ok, 'error) = result('ok, 'error);
let ok = a => Ok(a);
let error = e => Error(e);
let fromOption = (ma, ~error as [@ns.namedArgLoc] error) =>
  switch (ma) {
  | None => Error(error)
  | Some(right) => Ok(right)
  };
...
let unwrapLazy = (t, ~default as [@ns.namedArgLoc] default) =>
  switch (t) {
  | Ok(t') => t'
  | Error(_) => Lazy.force(default)
  };
let unwrapUnsafe = t => Belt.Result.getExn(t);
let unwrapError = (t, ~default as [@ns.namedArgLoc] default) =>
  switch (t) {
  | Ok(_) => default
  | Error(value) => value
  };
...
let values = t =>
  List.fold_right(
    (c, d) => map2(c, d, ~f=[@ns.namedArgLoc] (a, b) => [a, ...b]),
    t,
    Ok([]),
  );
let combine = (l: list(result('ok, 'error))): result(list('ok), 'error) =>
  TableclothList.foldRight(
    ~f=
        ...
        [@ns.namedArgLoc]
        (accum: result(list('ok), 'error), value: result('ok, 'error)):
        result(list('ok), 'error) =>
        map2(
          ~f=
            [@ns.namedArgLoc] (head: 'ok, list: list('ok)) =>
              [head, ...list],
          value,
          accum,
        ),
    ~initial=[@ns.namedArgLoc] Ok([]),
    l,
  );
let map = (t, ~f as [@ns.namedArgLoc] f) => Belt.Result.map(t, a => f(a));
let mapError = (t, ~f as [@ns.namedArgLoc] f) =>
  switch (t) {
...
let compare =
    (
      a: t('ok, 'error),
      b: t('ok, 'error),
      ~f as [@ns.namedArgLoc] (compareOk: ('ok, 'ok) => int),
      ~g as [@ns.namedArgLoc] (compareError: ('error, 'error) => int),
    )
    : int =>
  switch (a, b) {
